...
describe "CorrectAnswerBehavior" do
  describe "#was_correctly_answered" do
    ...
    it "if is in penalty box purses must no increase" do
      game =  CorrectAnswerBehavior.new
      purses = game.instance_variable_get(:@purses)[instance_variable_get(:@current_player)]
      # puts "afteeeeeeeeeeeeeee #{purses}"
      ...
      game.instance_variable_set(:@is_getting_out_of_penalty_box, false)
      game.was_correctly_answered
      # puts "despues #{game.instance_variable_get(:@purses)}"
      expect(game.instance_variable_get(:@purses)).to eq(purses)
    end
  end
  describe "#check_current_player" do
    ...
    it "current_player must change to zero when current_player" do
      game = CorrectAnswerBehavior.new
    ...
    it "current_player must increase by 1" do
      game = CorrectAnswerBehavior.new
